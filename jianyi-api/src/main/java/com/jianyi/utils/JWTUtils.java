...
import com.jianyi.auth.config.JWTParmConfig;
...
public class JWTUtils {
    ...
    public static TokenInfo tokenInfoByToken(String token) {
        ...
        return TokenInfo.builder().username(username).userId(userId).build();
    }

    public static boolean needRefresh(String token) {
        Date exp = JWTUtils.getExp(token);
        return System.currentTimeMillis() >= exp.getTime();
    }
    ...
    public static boolean loginExpire(String token) {
        if (Login_Interval == 0) {
            // 默认超时时间是8h
            Long minute = JWTParmConfig.expire;
            // 分钟换算成毫秒
